...
Xml Parser::parse()
{
    ...
    {
        if (!parse_declaration())//如果解析声明出错了,就抛个异常
        {
...
Xml Parser::parse_element()
{
    ...
    while (m_str[m_idx] != '\0')
    {
        ...
        {
            // parse xml attr
            string key = parse_element_attr_key();//分开解析键值
            skip_white_space();
            if (m_str[m_idx] != '=')
            {
    ...
    return elem;//这边使用了移动语义来进行优化
}

string Parser::parse_element_name()
{
    int pos = m_idx;
    if (isalpha(m_str[m_idx]) || (m_str[m_idx] == '_'))//如果是正常字符和下划线就合法
    {
        m_idx++;
...
{
    int pos = m_idx;
    while (m_str[m_idx] != '<')//一直解析到这个符号,这其中都是文本
    {
        m_idx++;
    }
    return m_str.substr(pos, m_idx - pos);
