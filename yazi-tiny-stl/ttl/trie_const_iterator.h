...
namespace ttl
{
    ...
    class TrieConstIterator
    {
        ...
        typedef TrieConstIterator<T> iterator;
        typedef std::vector<std::pair<Node *, int>> TrieStack;
    public:
        TrieConstIterator() : m_root(nullptr) {}
        TrieConstIterator(Node * root) : m_root(root) {}
        TrieConstIterator(Node * root, TrieStack & stack) : m_root(root)
        {
                m_stack.swap(stack);
        }
        ~TrieConstIterator() {};
    ...
    template <typename T>
    const T & TrieConstIterator<T>::operator * ()
    {
        string data;
    ...
    template <>
    const string & TrieConstIterator<string>::operator * ()
    {
        m_data.clear();
        for (int i = 0; i < m_stack.size(); i++)
        {
