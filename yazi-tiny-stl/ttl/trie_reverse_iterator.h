...
{
    template <typename T>
    class TrieReverseIterator
    {
        typedef TrieNode<T> Node;
        typedef TrieReverseIterator<T> iterator;
        typedef std::vector<std::pair<Node *, int>> TrieStack;
    public:
        TrieReverseIterator() : m_root(nullptr) {}
        TrieReverseIterator(Node * root) : m_root(root) {}
        TrieReverseIterator(Node * root, TrieStack & stack) : m_root(root)
        {
                m_stack.swap(stack);
        }
        ~TrieReverseIterator() {};
    ...
    template <typename T>
    T & TrieReverseIterator<T>::operator * ()
    {
        string data;
    ...
    template <>
    string & TrieReverseIterator<string>::operator * ()
    {
        m_data.clear();
        for (int i = 0; i < m_stack.size(); i++)
        {
