...
namespace ttl
{
    ...
    class BPTreeConstIterator
    {
        typedef BPTreeNode<T, D, t> Node;
        typedef BPTreeConstIterator<T, D, t> iterator;
    public:
        BPTreeConstIterator() : m_pointer(nullptr), m_offset(0) {}
        BPTreeConstIterator(Node * pointer, int offset) : m_pointer(pointer), m_offset(offset) {}
        ~BPTreeConstIterator() {}
