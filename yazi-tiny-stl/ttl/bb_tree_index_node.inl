...
BBTreeIndexNode<T, t>::BBTreeIndexNode(Node * parent) : BBTreeNode<T, t>(parent, false)
{
...
template <typename T, int t>
BBTreeIndexNode<T, t>::~BBTreeIndexNode()
{
    delete [] this->m_children;
}

template <typename T, int t>
typename BBTreeIndexNode<T, t>::Node * BBTreeIndexNode<T, t>::create(Node * parent)
{
    return new BBTreeIndexNode(parent);
}

template <typename T, int t>
void BBTreeIndexNode<T, t>::insert(const T & key)
{
    int i = 0;
    ...
    this->m_children[i]->insert(key);
}

template <typename T, int t>
void BBTreeIndexNode<T, t>::take_from_left()
{
    if (this->m_index == 0)
...
template <typename T, int t>
void BBTreeIndexNode<T, t>::take_from_right()
{
...
template <typename T, int t>
void BBTreeIndexNode<T, t>::merge()
{
    auto sibling = this->m_parent->m_children[this->m_index + 1];
...
template <typename T, int t>
bool BBTreeIndexNode<T, t>::remove(const T & key)
{
    int i = 0;
